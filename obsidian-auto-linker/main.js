...
function parseDictionary(content) {
  ...
  for (const line of lines) {
    ...
    if (!target.startsWith('[[') || !target.endsWith(']]')) { console.warn(`Auto Linker: invalid target format: ${target}`); continue; }
...
class AutoLinkerPlugin extends Plugin {
  async onload() {
    ...
    this.addCommand({
      id: 'auto-linker-convert-all',
      name: 'Vault全体をリンク変換',
      callback: async () => {
        try {
          const rules = await this.loadDictionary();
          if (!rules.length) { console.warn('Auto Linker: 辞書ノートが見つかりません'); return; }
          const files = this.app.vault.getMarkdownFiles();
          let total = 0, changed = 0;
          for (const file of files) {
            total++;
            if (file.path === this.settings.dictPath || file.path.endsWith('/' + this.settings.dictPath)) continue;
            const original = await this.app.vault.read(file);
            const { result } = processContent(original, rules);
            if (result !== original) {
              await this.app.vault.modify(file, result);
              changed++;
            }
          }
          new Notice(`Auto Linker: ${changed}/${total} ファイルを更新`);
        } catch (e) {
          console.error('Auto Linker: convert-all error', e);
          new Notice('Auto Linker: 一括変換でエラーが発生しました');
        }
      },
    });
  ...
  async convertFile(file) {
    ...
    // ★ 辞書ノートは処理しない
    if (file.path === this.settings.dictPath || file.path.endsWith('/' + this.settings.dictPath)) return;
  ...
  async loadDictionary() {
    try {
      ...
      if (!(file instanceof TFile)) { console.warn('Auto Linker: 辞書ノートが見つかりません'); return []; }
      const content = await this.app.vault.read(file);
      return parseDictionary(content);
    } catch (e) {
      console.warn('Auto Linker: 辞書読み込み失敗');
